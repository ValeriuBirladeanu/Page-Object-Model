...
class EmployeeListPage extends PimBasePage {
  constructor(page) {
    super(page);
    this.employeeInformationText = page.locator(
    ...
    this.employeeIdField = page
      .locator('[class="oxd-input oxd-input--active"]')
      .last();
    this.employmentStatusDropdown = page.locator(".oxd-select-text").first();
    this.employmentStatusOptions = page.locator(
      '[class="oxd-select-option"] span'
    );
    this.supervisorNameField = page
      .getByPlaceholder("Type for hints...")
      .nth(1);
    this.supervisorNameOption = page.locator(
      '[class="oxd-autocomplete-option"] span'
    );
  ...
  async selectDropdownOption(dropdown, optionsLocator, optionText) {
    ...
    const targetOption = optionsText.find(
      (text) =>
    ...
    if (targetOption) {
      ...
      throw new Error(
        `Option "${
          optionText.name || optionText.title
        }" not found in the dropdown.`
      );
    }
  }

  async fillEmployeeSearchFields(
    EmployeeFullName,
    employeeId,
    empStatus,
    SupervisorFullName,
